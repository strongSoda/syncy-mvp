...
const InfluencerProfile: React.FC<IInfluencerProfileProps> = ({influencer, setShowInfluencerProfile, setShowBookCall, setShowReachout}: IInfluencerProfileProps) => {
  ...
  return (
    <div className='influencer-profile'>
      ...
      <iframe title={influencer?.fullName} src={`${influencer?.profileUrl ? influencer?.profileUrl[-1]==="/" ? influencer?.profileUrl : influencer?.profileUrl + '/' : `https://www.instagram.com/${encodeURI(influencer?.instagram_username ? influencer?.instagram_username : influencer?.instagramUsername)}/`}embed`} name="myiFrame" scrolling="yes" frameBorder="0" height="900" width="100%" allowFullScreen={true}></iframe>
